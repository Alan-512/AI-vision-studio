    ...
    >
      ...
      {(isGenerating || isPending) ? (
        <div className="w-full h-full flex flex-col items-center justify-center bg-dark-panel p-4 text-center space-y-3 relative overflow-hidden">
          ...
          <p className="text-[10px] text-gray-500 mt-2 line-clamp-3 px-2 relative z-10 font-medium opacity-70">
            {asset.prompt}
          </p>
        </div>
      ) : isFailed ? (
          ...
          <div className="flex gap-2 justify-end z-20 flex-wrap">
            {isTrashMode ? (
              <>
                ...
                <button onClick={handleDelete} className="p-1.5 bg-red-500/80 hover:bg-red-600 rounded-lg text-white backdrop-blur-md transition-colors" title="Delete Permanently"><Trash2 size={16} /></button>
              </>
            ) : (
              <>
                {onToggleFavorite && (
      ...
      {/* Bookmark-style favorite indicator in top-right corner */}
      {asset.isFavorite && !isTrashMode && !isGenerating && !isPending && !isFailed && (
        <div className="absolute -top-1 right-3 pointer-events-none">
          <div className="relative">
            <Bookmark size={32} className="text-amber-500 fill-amber-500 drop-shadow-lg" />
          </div>
        </div>
      )}
    </div>
