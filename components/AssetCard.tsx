...
export const AssetCard: React.FC<AssetCardProps> = ({ 
  ...
  const handleClick = () => {
     ...
     if (isSelectionMode && onToggleSelection) onToggleSelection(asset);
     else onClick(asset);
  };
    ...
    >
      ...
      {(isGenerating || isPending) ? (
         <div className="w-full h-full flex flex-col items-center justify-center bg-dark-panel p-4 text-center space-y-3 relative overflow-hidden">
             ...
             {isGenerating ? (
                 <div className="relative z-10 flex flex-col items-center">
                    ...
                    <div className="text-xs font-bold text-brand-400 tracking-wide animate-pulse">
                      {t('task.processing')}
                    </div>
                 </div>
             ) : (
                 <div className="relative z-10 flex flex-col items-center">
                    ...
                    <div className="text-xs font-bold text-yellow-500 tracking-wide">
                      {t('task.queued')}
                    </div>
                 </div>
             )}
         ...
         <div className="w-full h-full flex flex-col items-center justify-center bg-red-900/10 p-4 text-center space-y-2">
             ...
             <p className="text-xs font-bold text-red-400">{t('task.failed')}</p>
      ...
      {!isSelectionMode && !isGenerating && !isPending && !isFailed && (
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3 z-10">
          ...
          <div className="flex gap-2 justify-end z-20 flex-wrap">
            {isTrashMode ? (
              <>
                 <button onClick={handleRestore} className="p-1.5 bg-green-500/80 hover:bg-green-600 rounded-lg text-white backdrop-blur-md transition-colors" title="Restore"><RotateCcw size={16} /></button>
                 <button onClick={handleDelete} className="p-1.5 bg-red-500/80 hover:bg-red-600 rounded-lg text-white backdrop-blur-md transition-colors" title="Delete Permanently"><X size={16} /></button>
              </>
            ) : (
              <>
                {onToggleFavorite && (
                ...
                {asset.type === 'IMAGE' && onAddToChat && (
                  <button onClick={handleAddToChat} className="p-1.5 bg-indigo-500/80 hover:bg-indigo-500 rounded-lg text-white backdrop-blur-md transition-colors" title="Add to Assistant"><MessageSquarePlus size={16} /></button>
                )}
            ...
            <button type="button" className="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-white backdrop-blur-md transition-colors" title="Inspect"><ZoomIn size={16} /></button>
          </div>
        </div>
      )}
