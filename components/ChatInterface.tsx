...
import { Send, User, Sparkles, ChevronDown, ChevronRight, BrainCircuit, Zap, X, Box, ChevronUp, Copy, Check, Plus, Layers, Clock, MonitorPlay, Palette, Film, RefreshCw, Trash2, Bot, Square, Crop, Hammer, CheckCircle2, Globe, ScanFace, Brain, CircuitBoard, Wrench, Image as ImageIcon, CircleDashed, Terminal, Clapperboard, AudioWaveform, Move3d } from 'lucide-react';
import { ChatMessage, ChatModel, GenerationParams, ImageStyle, ImageResolution, AppMode, ImageModel, VideoResolution, VideoDuration, VideoModel, VideoStyle, AspectRatio, Project, SmartAsset, APP_LIMITS, AgentAction } from '../types';
...
const AgentStepItem: React.FC<{ step: AgentStep; isLast: boolean; isThinking: boolean }> = ({ step, isLast, isThinking }) => { 
  ...
  return (
    <div className={`flex gap-4 ${isUser && !isSystem ? 'flex-row-reverse' : ''} group animate-in fade-in slide-in-from-bottom-2`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
          isSystem && isFeedback ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
